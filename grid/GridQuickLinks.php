...
class GridQuickLinks extends Widget
{
    ...
    public function init()
    {
        ...
        if (!$this->defaultOptions) {
            $this->defaultOptions = [
                'all' => ['label' => Yii::t('art', 'All'), 'filterWhere' => []],
                'active' => ['label' => Yii::t('art', 'Active'), 'filterWhere' => ['status' => 1]],
                'inactive' => ['label' => Yii::t('art', 'Inactive'), 'filterWhere' => ['status' => 0]],
            ];
        }
        ...
        if (!$this->links) {
            ...
            foreach ($this->options as $option) {
                if (($this->showCount)) {
                    ...
                    if ((ArtHelper::isImplemented($model, OwnerAccess::CLASSNAME)
